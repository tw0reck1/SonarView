...
class StrokePlainSonarView : RotaryView, Sonar {

    companion object {
        private const val DEFAULT_COLOR = 0xff03CC02.toInt()
        private const val ARC_MASK = 0xffffffff.toInt()
    ...
    private var outerBorder = DEFAULT_OUTER_BORDER
    ...
    private fun drawArc(canvas: Canvas, centerX: Float, centerY: Float, radius: Float) {
        ...
        val arcPoint = getPointOnCircle(centerX, centerY, radius, scannerAngle)

        canvas.drawLine(centerX, centerY, arcPoint.x, arcPoint.y, arcPaint)
    }
    ...
    private fun getSonarBitmap(width: Int, height: Int): Bitmap? {
        ...
        circleCanvas.drawCircle(radius, radius, (radius - padding) * 0.75f, thinStrokePaint)
        circleCanvas.drawCircle(radius, radius, (radius - padding) * 0.5f, thinStrokePaint)
        circleCanvas.drawCircle(radius, radius, (radius - padding) * 0.25f, thinStrokePaint)

        for (i in 0 until LINE_COUNT) {
            val angle = i * LINE_ANGLE
            val start = getPointOnCircle(radius, radius, radius, angle)
            val end = getPointOnCircle(radius, radius, 0f, angle)
